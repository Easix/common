...
    public static class AesCrypto
    {
        ...
        #region 参数是byte[]类型的
        public static byte[] Encrypt_cbc(byte[] palinData, string key, string iv)
        {
            if(palinData == null) return null;
            if(!(CheckKey(key) && CheckIv(iv))) return null;
            byte[] toEncryptArray = palinData;
            var rm = new RijndaelManaged
            {
                IV = Encoding.UTF8.GetBytes(iv),
                Key = Encoding.UTF8.GetBytes(key),
        ...
        public static byte[] Decrypt_cbc(byte[] encryptedData, string key, string iv)
        {
            if(encryptedData == null) return null;
            if(!(CheckKey(key) && CheckIv(iv))) return null;
            byte[] toEncryptArray = encryptedData;
            var rm = new RijndaelManaged
            {
                IV = Encoding.UTF8.GetBytes(iv),
                Key = Encoding.UTF8.GetBytes(key),
